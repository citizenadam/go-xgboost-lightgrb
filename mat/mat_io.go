...
func DenseMatFromLibsvm(reader *bufio.Reader, limit int, skipFirstColumn bool) (*DenseMat, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("unable to parse libsmv format to dense matrix: %w", err)
	}
	return mat, nil
}
...
func CSRMatFromLibsvm(reader *bufio.Reader, limit int, skipFirstColumn bool) (*CSRMat, error) {
	...
	err := readFromLibsvm(reader, limit, skipFirstColumn, f)
	if err != nil {
		return nil, fmt.Errorf("unable to parse libsmv format to sparse matrix: %w", err)
	}
	return mat, nil
}
...
func DenseMatFromCsv(reader *bufio.Reader,
	...
	if err != nil {
		return nil, fmt.Errorf("unable to parse csv format to dense matrix: %w", err)
	}
	return mat, nil
}
...
func readFromLibsvm(reader *bufio.Reader, limit int, skipFirstColumn bool, f libsvmRowFunc) error {
	...
	for {
		...
		for col := startIndex; col < len(tokens); col++ {
			...
			if err != nil {
				return fmt.Errorf("can't convert to float %s: %w", pair[0], err)
			}
			value, err := strconv.ParseFloat(pair[1], 64)
			if err != nil {
				return fmt.Errorf("can't convert to float %s: %w", pair[1], err)
			}
	...
	for {
		...
		for col := startIndex; col < len(tokens); col++ {
			...
			if len(tokens[col]) == 0 {
				...
				if err != nil {
					return fmt.Errorf("can't convert to float %s: %w", tokens[col], err)
				}
				value = fvalue
			}
