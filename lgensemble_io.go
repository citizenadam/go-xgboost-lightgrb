...
func LGEnsembleFromReader(reader *bufio.Reader, loadTransformation bool) (*Ensemble, error) {
	...
	for i := 0; i < nTrees; i++ {
		...
		if err != nil {
			return nil, fmt.Errorf("error while reading %d tree: %w", i, err)
		}
...
func unmarshalNode(raw []byte) (any, error) {
	node := &lgNodeJSON{}
...
func unmarshalTree(raw []byte) (lgTree, error) {
	...
	createCategoricalNode := func(nodeJSON *lgNodeJSON) (lgNode, error) {
		...
		if nBits == 0 {
			...
			if err != nil {
				return node, fmt.Errorf("can't convert %s: %w", tokens[0], err)
			}
			catIdx = uint32(value)
			catType = catOneHot
		} else {
			...
			for i, valueStr := range tokens {
				...
				if err != nil {
					return node, fmt.Errorf("can't convert %s: %w", valueStr, err)
				}
