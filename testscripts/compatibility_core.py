...
""".format(datetime.now().strftime("%Y-%m-%d %H:%M"))
    ...
    def report(self) -> str:
        ...
        for library in libraries:
            ...
            for case in cases:
                version_map = {
                    outcome.env.version: outcome.is_success
                    for outcome in self.outcomes
...
require "github.com/citizenadam/go-xgboost-lightgrb" v0.0.0
replace "github.com/citizenadam/go-xgboost-lightgrb" v0.0.0 => "{self.leaves_path}"
""")
    ...
    def compare_matrices(
        ...
        self.logger.info(
            f"Compare matrices from files: '{matrix1_filename}' and '{matrix2_filename}'"
        )
        ...
        if number_of_mismatches > max_number_of_mismatches_ratio * m1.size:
            raise CompareError(
                f"number of mismatches = {number_of_mismatches} (maximum allowed {max_number_of_mismatches_ratio * m1.size}"
            )
